...
需要的类型画、配色为这个读者和这个场合定。**本 skill 里没有可以挑的内置风格**——
以前有六种，删了，因为「从六个里挑一个」不是设计，
理由见 `references/report-styles.md` 第八节。
...
**写第一行 HTML 之前，把五段写下来，然后照着它写，不要边写边改主意。**
...
边写 CSS 边选颜色一定会漂：第三屏的灰和第一屏的灰不是同一个灰，强调色用到四种，
间距冒出 13/15/18 三个差不多的值。先定 token 再写，可用的值就只有那几个。
**它约束的不是审美，是一致性**——那是「有设计」和「没设计」之间，
读者最先看出来的那条线。

计划要落进产出物：写成 HTML 头部注释，颜色定义成 CSS 变量，别把色值散在组件里。
（顺带，打印样式只要重定义变量就有了。报告读者会打印，**但不要做深色主题**——
报告不跟随读者的系统主题，做两套只是把决策翻倍。）

**写完先自己审一遍：里面有没有哪一条，换个题目也照样能用？**
有就改掉，并在说明理由时讲清改了什么。十五条最常见的默认倾向
（米黄底衬线、四个圆角 KPI 大卡、章节标题挂 emoji、渐变柱子、深蓝配浅灰当专业感……）
列在 `references/report-styles.md` 第五节，**逐条对照，不是读一遍就算**。
